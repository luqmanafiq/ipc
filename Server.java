	...
	public void receiveFileWithAckLost(float loss) {
		try {
			...
			while (currentTotal < totalBytes) {
				...
				if (x == dataSeg.getChecksum()) {
					System.out.println("SERVER: Calculated checksum is " + x + "  VALID");
					//implement data sequence
					// Valid data segment with the expected sequence number
					// Send ACK for this segment
					if (dataSeg.getSq() == expectedSequenceNumber) {
						Segment ackSeg = new Segment();
						...
						//simulate ack based on loss probability
						if (isLost(loss)) {
							System.out.println("SERVER: Simulating ACK loss");
							System.out.println("SERVER: Data segment content is written to the file.");
							myWriter.write(dataSeg.getPayLoad());
						} else {
							/* Send the Ack segment */
							socket.send(replyPacket);
						...
						currentTotal = currentTotal + dataSeg.getSize();
						expectedSequenceNumber = 1 - expectedSequenceNumber; // Alternate expected sequence number

					} else {
						System.out.println("SERVER: Duplicate data segment with sq " + dataSeg.getSq());
						System.out.println("SERVER: Sending an ACK with sq " + (1 - expectedSequenceNumber));
						Segment ackSeg = new Segment();
						ackSeg.setSq(1 - expectedSequenceNumber);
						ackSeg.setType(SegmentType.Ack);
